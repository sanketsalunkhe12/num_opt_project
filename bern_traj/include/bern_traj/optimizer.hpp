...
{
    public:
        DQPSolver(const int j); // takes in solver index j for ease of mapping
        ...
        void compute_residuals(); // perform calculation of residuals in place
        ...
        int local_index;
        Eigen::Ref<const MatrixXd> Q; // PSD Objective Function matrix
        Eigen::Ref<const MatrixXd> A; // Equality+Inequality constraints
        ...
        Eigen::Ref<const VectorXd> lower; 
        Eigen::Ref<const VectorXd> upper; 
        ...
        Eigen::MatrixXd lhs;
        Eigen::MatrixXd rhs;
        Eigen::LDLT<MatrixXd> ldlt_solver;
...
{
    public:
        DistributedOptimizer(const int &nSolvers);
        ...
        // Setters and getters
        std::vector<float> getPrimalResiduals(); // Just a vector of x?
        std::vector<float> getDualResiduals(); // just a vector of y?

        // Public member variables
        int m_nSolvers = 1; // default to a single solver (which should still work...)
        std::vector<upDQPSolver> m_solvers;
